...
//白名单
const verifyOrigin = async (ctx, next) => {
    const validOrigins = VALID_ORIGINS;
...
module.exports = async (ctx, next) => {
    ...
    // ctx.set("Access-Control-Allow-Origin", "http://localhost:7999"); // 只允许指定域名http://localhost的请求
    await verifyOrigin(ctx, next)
